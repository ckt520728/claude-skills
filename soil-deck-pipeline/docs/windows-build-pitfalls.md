...
**根因（重要更正）：** **Pillow 並沒有壞、DLL 也沒少。** 真正原因是
`<anaconda>\Library\bin`（放 `libjpeg.dll` / `zlib` / `libpng16.dll` 等相依 DLL 的目錄）
**沒有被加到 PATH**——這個目錄只有在「啟用 conda 環境」時才會掛上。直接呼叫 `python`
（未 `conda activate`）時就找不到相依 DLL，`_imaging` C 擴充載入失敗。
> 同一根因也會讓 `pip` 報 `ssl module is not available`（`libssl/libcrypto` 也在這個 bin 裡）。
> **驗證方式：** `where python` 看是不是 anaconda 的；把 `Library\bin` 加進 PATH 後 `from PIL import Image` 立刻成功。

**❌ 不要做：** 重灌 Anaconda / `conda update` —— 問題不在安裝，重灌完只要還是「未啟用就跑 python」一樣會壞。

**✅ 首選解法（治本）—— 把 conda 的 DLL 目錄掛回去：**

1. **腳本級（推薦給自動化 / Claude Code）** — Python 3.8+，在 `import PIL` / `import pptx` **之前**加一行：
   ```python
   import enable_conda_dlls        # 自動偵測 conda 並掛 DLL（見 scripts/）
   from PIL import Image           # OK
   from pptx import Presentation   # OK
   ```
   助手見 [`scripts/enable_conda_dlls.py`](../scripts/enable_conda_dlls.py)。
   > ⚠️ **實測重點：光靠 `os.add_dll_directory(...Library\bin)` 不夠**——Pillow 的 `_imaging`
   > 會「傳遞相依」libjpeg/zlib 等 DLL，安全搜尋路徑涵蓋不到。必須**同時**注入
   > `os.environ["PATH"]`（傳統 loader 才解析得到傳遞相依）。`enable_conda_dlls` 兩件事都做了，
   > 所以直接用它，不要只抄 `add_dll_directory` 那一行。
2. **互動級** — 從開始選單開「Anaconda Prompt」（自動 `conda activate base`），或在一般 PowerShell 跑一次 `conda init powershell` 後 `conda activate base`。
3. **臨時 shell** — `set "PATH=C:\Users\User\anaconda3\Library\bin;%PATH%"`（cmd）。

**🩹 退路（繞道，當你不想／不能動環境時）：**
- **影像壓縮** → 改用 **PowerShell + .NET System.Drawing**（見 [`scripts/compress_images.ps1`](../scripts/compress_images.ps1)），完全不碰 Pillow。
- **產 PPTX** → 在 `import pptx` 前注入最小 PIL stub（見 [`scripts/pptx_pil_stub.py`](../scripts/pptx_pil_stub.py)），用 `struct` 自己讀 PNG/JPEG header。
  > 退路適用於「跨機器、不確定 anaconda 路徑、或環境本身真的損壞」的場合；能掛 DLL 目錄時優先用首選解法。
...
**✅ 首選解法：** 跟坑 1 同根因（PATH 沒掛 DLL 目錄）。先試
`os.add_dll_directory(r"...\anaconda3\Library\bin")` 或 `import enable_conda_dlls`，
能掛上的話就能直接用真正的 python-pptx + Pillow，**不需要 stub**。

**🩹 退路（環境真的動不了時）：** 在 `import pptx` 前注入 stub，需涵蓋 **`PIL.Image`、`PIL.ImageFont`、`PIL.ImageDraw`** 三個子模組（python-pptx 的 text/chart 模組都會 import）。`Image` stub 要提供：
...
> **「PIL 的 DLL 找不到」**和**「PS 5.1 把 UTF-8 當 ANSI」**。
>
> - 前者**不是 Pillow 壞掉**，而是 conda 的 `Library\bin` 沒掛上 PATH——
>   **治本**用 `import enable_conda_dlls`（add_dll_directory + PATH 兩者並用），
>   **退路**才走 System.Drawing 壓圖 + python-pptx stub。**千萬別為此重灌 Anaconda。**
> - 後者→ 腳本全 ASCII + 中文走 UTF-8 sidecar。
>
> 其餘大多是這兩個根因的衍生症狀。
