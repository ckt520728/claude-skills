...
- **PIL/python-pptx 報 `DLL load failed`** → **首選治本**：在 import 前 `import enable_conda_dlls`（見 `scripts/`，會掛 conda 的 `Library\bin` 到 add_dll_directory + PATH），這樣能直接用真正的 Pillow / python-pptx。**不是 Pillow 壞掉、不要重灌 Anaconda。**
- 不想／不能動環境時的**退路**：影像壓縮用 `scripts/compress_images.ps1`（System.Drawing）；產 PPTX 用 `scripts/pptx_pil_stub.py`（import 前注入 PIL stub）。
